    ...
    public function insert(XoopsObject $obj, $force = false)
    {
        ...
        if ($obj->isNew()) {
            ...
            $sql = sprintf('INSERT INTO %s (id, application_id, tax_class_id, display_order, base_currency_id, package_group_id,
                                      ...
                                      VALUES (%u, %u, %u, %u, %u, %u, %u, %s, %s, %f, %f, %u, %u, %u, %u, %u, %u, %s, %u, %u, %u, %s)', $this->_db->prefix($this->_dbtable), $id, $application_id, $tax_class_id, $display_order, $base_currency_id, $package_group_id, $sample_package_group_id,
                           $this->_db->quoteString($item_code), $this->_db->quoteString($item_description), $unit_price, $old_unit_price, $min_unit_count, $max_access, $max_days, $expires, $add_to_group, $add_to_group2, $this->_db->quoteString($item_rich_description), $enabled, $group_expire_date,
                           $group_expire_date2, $this->_db->quoteString($extra_instructions));
        } else {
            $sql = sprintf('UPDATE %s SET application_id = %u, tax_class_id = %u, display_order = %u, base_currency_id = %u,
                                        ...
                                        item_rich_description = %s, enabled = %u, group_expire_date = %u, group_expire_date2 = %u, extra_instructions = %s WHERE id = %u', $this->_db->prefix($this->_dbtable), $application_id, $tax_class_id, $display_order, $base_currency_id, $package_group_id,
                           $sample_package_group_id, $this->_db->quoteString($item_code), $this->_db->quoteString($item_description), $unit_price, $old_unit_price, $min_unit_count, $max_access, $max_days, $expires, $add_to_group, $add_to_group2, $this->_db->quoteString($item_rich_description),
                           $enabled, $group_expire_date, $group_expire_date2, $this->_db->quoteString($extra_instructions), $id);
        }
        //echo $sql;
        // Update DB
        if (false != $force) {
